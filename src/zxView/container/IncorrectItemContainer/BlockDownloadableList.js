                ...
                if (!$.isEmptyObject(testList)) {
                    ...
                    for (let j = 0; j < incorrectItem.length; j++) {
                        ...
                        if (incorrectItem[j].quiz_cat !== lastLable) {
                            ...
                            text.push(`<span style="float: right">[E.<span style="font-weight:700;">${(j + 1)}</span>]&nbsp;<span style="color: #1d7d74"><span style="font-weight: 700;">测试名:</span><span style="font-size:10px;">${incorrectItem[j].bank_test_name}</span></span>${incorrectItem[j].text}</span>`);
